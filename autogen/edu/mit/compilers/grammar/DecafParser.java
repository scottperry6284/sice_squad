...
inputState.guessing--;
	...
	protected final void expr_nonroot() throws RecognitionException, TokenStreamException {
		
		traceIn("expr_nonroot");
		try { // debugging
			returnAST = null;
			ASTPair currentAST = new ASTPair();
			AST expr_nonroot_AST = null;
			...
			try {      // for error handling
				...
				expr_nonroot_AST = (AST)currentAST.root;
			}
			catch (RecognitionException ex) {
				if (inputState.guessing==0) {
					reportError(ex);
			...
			returnAST = expr_nonroot_AST;
		} finally { // debugging
			traceOut("expr_nonroot");
		}
	}
...
inputState.guessing--;
	...
	public final void len() throws RecognitionException, TokenStreamException {
		...
		try { // debugging
			...
			try {      // for error handling
				...
				match(RPAREN);
				if ( inputState.guessing==0 ) {
					len_AST = (AST)currentAST.root;
					len_AST = (AST)astFactory.make( (new ASTArray(2)).add(astFactory.create(AST_len,"AST_len")).add(len_AST));
					currentAST.root = len_AST;
					currentAST.child = len_AST!=null &&len_AST.getFirstChild()!=null ?
						len_AST.getFirstChild() : len_AST;
					currentAST.advanceChildToEnd();
				}
				len_AST = (AST)currentAST.root;
			}
